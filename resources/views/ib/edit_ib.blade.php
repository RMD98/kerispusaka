...
<div class="bg-white w-full shadow rounded-2xl p-4">
    ...
    <div class="flex items-center justify-between mb-4">
        <h3 class="text-xl font-semibold text-gray-800">Edit Inseminasi Buatan</h3>
    </div>

    {{-- Table --}}
    <div class="overflow-x-auto">
    <form action="{{ route('ib.update', $data->id_ib) }}" method="POST" class="flex-1 overflow-auto space-y-6">
                @csrf
                @method('PUT')
                    <input type="hidden" name="id" value="{{$data->id_ib}}" readonly>
                <div>
                ...
                <div>
                    <label for="staff" class="block text-sm font-medium text-gray-700">No Dokumen</label>
                ...
                <div>
                    <label for="pejantan" class="block text-sm font-medium text-gray-700">Pejantan (Straw)</label>
                    <select id="pejantan" name="pejantan" class="w-full rounded-lg shadow-sm border border-gray-300" required>
                    <option value="{{$data->pejantan}}" selected>{{$data->pejantan}}</option>
                ...
                <div>
                    ...
                    <select id="status" name="status" class="w-full rounded-lg shadow-sm border border-gray-300">
                        ...
                        <option value="Belum Ada Hasil"  {{$data->hasil == 'Belum Ada Hasil' ? ' selected ' : '' }}>Belum Ada Hasil</option>
                        <option value="sukses" {{$data->hasil == 'sukses' ? ' selected ' : '' }}>Inseminasi Berhasil</option>
                        <option value="gagal"  {{$data->hasil == 'gagal' ? ' selected ' : '' }}>Inseminasi Gagal</option>
                    </select>
                </div>

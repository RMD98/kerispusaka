...
<div class="bg-white w-full shadow rounded-2xl p-4">
    ...
    <div class="flex items-center justify-between mb-4">
        <h3 class="text-xl font-semibold text-gray-800">Edit Kelahiran</h3>
    </div>

    {{-- Table --}}
    <div class="overflow-x-auto">
    <form action="{{ route('kelahiran.update', $data->id_kelahiran)}}" method="POST" class="flex-1 overflow-auto space-y-6">
                @csrf
                 <input type="hidden" name="id" value="{{$data->id_kelahiran}}" readonly>
                <div>
                ...
                <div>
                    ...
                    <input name="nama" id="nama" type="text" placeholder="Nama Sapi Lahir" required value="{{$data->nama}}"
                ...
                <div>
                    ...
                    <input type="date" name="tanggal" id="tanggal" required value="{{date('Y-m-d',strtotime($data->created_at))}}"
                           class="mt-1 block w-full border border-gray-300 rounded-lg shadow-sm px-4 py-2 focus:ring-blue-500 focus:border-blue-500">
                </div>

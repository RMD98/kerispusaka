...
<div class="bg-white w-full shadow rounded-2xl p-4">
    ...
    <div class="overflow-x-auto">
        <form action="{{ route('penyakit.update', $data->id_penyakit) }}" method="POST" class="flex-1 overflow-auto space-y-6" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div>
            ...
            <div>
                ...
                <input type="text" name="peternak" id="peternak" required readonly value="{{$data->id_peternak}}"
            ...
            <div>
                ...
                <select id="sapi" name="sapi" class="w-full rounded-lg shadow-sm border border-gray-300">
                    <option value="{{$data->ear_tag}}" selected>{{$data->ear_tag}} - {{$data->nama_sapi}}</option>
                </select>
            </div>
            <div>
            ...
            <div>
                    ...
                    <input type="datetime-local" name="tanggal" id="tanggal" required value="{{date('Y-m-d\TH:i', strtotime($data->created_at))}}"
